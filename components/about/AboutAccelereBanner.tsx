    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center px-6">
        <h2
          ...
          style={{ fontSize: "clamp(52px, 10vw, 140px)", letterSpacing: "-0.02em" }}
        >
      ...
      <div className="absolute bottom-[6%] left-0 right-0 flex flex-col items-center gap-[clamp(6px,1vw,14px)]">
        <p
          className="m-0 text-white/80 uppercase text-center leading-snug"
          style={{ fontFamily: "var(--font-poppins), sans-serif", fontSize: "clamp(8px,0.85vw,13px)", letterSpacing: "0.22em" }}
        >
          BEHIND EVERY GREAT DRIVER,<br />SOMEONE BELIEVED FIRST.
        </p>
        <div className="flex items-center gap-[clamp(10px,1.5vw,22px)]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={THE_GRID_LOGO}
            alt="The Grid Agency"
            style={{ height: "clamp(18px,1.8vw,30px)" }}
            className="w-auto object-contain"
            draggable={false}
          />
          <span className="text-white/40" style={{ fontSize: "clamp(12px,1.1vw,18px)" }}>×</span>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={COME_LOGO}
            alt="Côme Ensemble"
