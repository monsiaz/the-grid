#!/usr/bin/env node
/**
 * Seed the services-page global with the full content from COLLECTE DES TEXTES.
 * Runs against the local dev server or production via BASE_URL env var.
 */
...
const SERVICES_DATA = {
  ...
  valueHeading: "WHERE PERFORMANCE CREATES",
...
async function run() {
  ...
  // Update services-page global via Payload REST API
  const r = await fetch(`${BASE}/api/globals/services-page`, {
    method: "POST",
    headers,
    body: JSON.stringify(SERVICES_DATA),
  });

  if (!r.ok) {
    const text = await r.text();
    console.error("Error:", r.status, text.slice(0, 400));
    return;
  }
  const d = await r.json();
  console.log("✅ services-page updated, fields:", Object.keys(d.doc || d).join(", "));
}
